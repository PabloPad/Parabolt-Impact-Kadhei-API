...
public class SubtaskController {
	...
	public ResponseEntity<?> deleteSubtaskById(@PathVariable int id) {
		
		Optional<Subtask> foundSubtask = subtaskService.deleteSubtask(id);
		if(foundSubtask.isPresent()) {
			return new ResponseEntity<Subtask>(foundSubtask.get(),HttpStatus.OK);
		}
		else {
	...
	public ResponseEntity<?> updateSubtaskById(@PathVariable int id, @RequestBody Subtask newSubtask) {
		
		Optional<Subtask> foundSubtask = subtaskService.updateSubtask(id, newSubtask);
		if(foundSubtask.isPresent()) {
			return new ResponseEntity<Subtask>(foundSubtask.get(),HttpStatus.OK);
		}
		else {
